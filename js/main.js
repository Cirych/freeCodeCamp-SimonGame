window.onload = (function() {
	...
	function displayCount(count) {
		config.display.innerHTML = (count < 10)?
			'0'+count : count;
	};
	...
	function playSound(index) {
		config.gainNodes[index].gain
			.linearRampToValueAtTime(volume, audioContext.currentTime + ramp);
	...
	function pushButton(index) {
		if(!config.lock) {
			...
			if(index === config.sequence[config.index]
				&& config.index < config.stepCount) {
				playSound(index);
